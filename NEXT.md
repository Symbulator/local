...
Roberto said *"Please fix the note."* The note was right. The angle is
-36.20493°, which is -36.20 at four figures. `sp.N(x, 4)` does not
round to four decimal digits: it evaluates `x` at a *binary* working
precision of about four digits (13 bits), and the nearest 13-bit float
to -36.20493 prints as -36.21. **The app's own `_round_expr` is the
same call**, and its `_polar_format` and `aa` mini-tool the same again,
so the app shows -36.21 too -- the Rounding setting has been off by
one in the last digit, sometimes, since it existed.

**Measured, not guessed.** Every entry with a numeric `rounding:` run
through the package (120 entries, 2,269 numeric answers) and each
answer rounded both ways: **twelve differ**, all twelve the app's
rule wrong -- B11 7.10's `v_r8` 6.545 shown as 6.54; AS7 PP10.1's
`s_e` 7.0755j as 7.076j; AS7 10.13's `v_l1`, `s_ro` and `ap_ro`; AS7
10.14's `s_r3` -5.2295 as -5.229; AS7 12.4's `z_eb0` 1.7265 as 1.727;
AS7 PP12.10's `z_eb0` -5.7735 as -5.773; AS7 12.12's `i_rlc`, where
the app also *dropped a digit* (-0.00991 for -0.009909); Lesson 10's
live-windings `v_3` and `v_rl` 4.9505 as 4.951; and the showcase's
`v_r6` -16.655 as -16.66. Plus the angle that started it, which the
sweep does not count because it goes through the polar formatter.

**Two false starts, worth keeping.** The first decimal version used
Python's own formatting, which rounds ties *to even*: 31.25 became
31.2, and ten of the tutorial's printed answers changed -- a book
prints 31.3, and so does the app, by the luck of binary. So ties go
half away from zero (`decimal.ROUND_HALF_UP`). The second read the
float by its shortest repr, and a computed -0.00585 -- the double just
below the tie -- became -0.0058; the value is now read at fifteen
decimal digits, a double's own precision, and anything past that is
treated as the noise it is. After both, the sweep's differences are
exactly the twelve genuine ones and nothing else.

**The package half** (`symbulator/_display.py`, `round_sig` and
`_round_float`): `polar()` and `Result.rounded()` go through it; five
tests pin the tie, the noise case and two of the twelve; suite 413.
The monograph notebook rebuilt and redeployed to learn (192,016 b,
hash-verified), reading -36.20 as the note does. **Unreleased**: it
reaches PyPI as 0.5.31 on Roberto's word, with the usual train.

**The app half** (Roberto: *"Do 1 and 2."*, the same evening).
`symbulator_ui.py`'s `_round_expr` (four call sites), `_polar_format`'s
`_num` and the `aa` mini-tool's `rounded` (which keeps its two extra
digits) all call the package's `round_sig` now; the server pin moved
to `>=0.5.31`, which is what makes the import safe. Verified through
the real app: `verify_lesson.py The_Monograph` prints
`iraa=2.350∠-36.20° A` where it printed -36.21, and `_round_expr`
gives 4.950, 31.3 and -0.009909 on the sampled cases. **A correction
to the paragraph this replaces:** `tools/*.expected.json` are *not*
generated by the app -- they hold the book's printed answers, which
the harness shows beside the app's for a person to read -- so nothing
needed regenerating; the twelve answers moved toward those files, not
away from them. Nothing in the docs changes.

**The release, 0.5.31.** Editable install refreshed (the
version-metadata test), suite 413 unpiped, `python -m build`, `twine
check` PASSED, upload; wheel sha256 `397c5404…`, verified against
PyPI. The wheel into `vendor/`, 0.5.30 deleted, the three pins moved,
cache **v157** (each read back in `git diff`), `build_local.py`,
`build_zip.py` (**31,832,182 b**), `stage_install_site.py`, both
deploys with all hash checks passing; live `sw.js` says v157 and names
the 0.5.31 wheel, the served wheel hashes equal to PyPI's, and the
served `symbulator_ui.py` carries `round_sig`. X merged as **X7**
